...
public class LDTKTileset {
    ...
    public LDTKTileset(string identifier, Subtexture atlas, Point2 tileCount, int tileSize, TilesetCustomData[] customData) {
        ...
        foreach (TilesetCustomData entry in customData) {
            LDTKTileElement? element = Get(entry.ID);
            // Console.WriteLine($"{entry.ID} {GetTileCoord(entry.ID)}");
    ...
    ICollider? ColliderFromData(string data) {
        ...
        switch (args[0]) {
            case "FULL": { return DefaultCollider; }
            case "RECT": {
                    if (
                        args.Length != 5 ||
